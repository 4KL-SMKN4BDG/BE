...
const catchResponse = (err: any, req: Request, res: Response, _next: NextFunction) => {
  ...
  if (err.code == 'P2003') {
    ...
    data = err.details.map((det: any) => det.message);
  ...
  if (req.files) {
    if (typeof req.files === 'object')
      ...
      req.files.forEach((file: any) => uploaded.push(file));
  }
