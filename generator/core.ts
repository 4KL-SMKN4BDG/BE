...
import ${name}Controller from "./${nameL}.controller.ts";
import ${name}Validator from "./${nameL}.validator.ts";
...
`,
  },
  {
    ...
    content: `import BaseController from "../../base/controller.base.ts";
import { NotFound } from "../../exceptions/catch.exception.ts";
import ${name}Service from "./${nameL}.service.ts";
...
`,
  },
  {
    ...
    content: `import BaseService from "../../base/service.base.ts";
...
class ${name}Service extends BaseService {
  ...
  findAll = async (query: any) => {
    ...
    const data = await this.db.${modelName}.findMany({ ...q as {[key: string]: never} });
